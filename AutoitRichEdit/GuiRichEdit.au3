...
Func __RichCom_Object_AddRef($pObject)
	...
	; __GCR_Debug("AddRef pObj: " & $pObject)
	; __GCR_Debug("AddRef pObj1: " & DllStructGetData($pObject, 1))
	; __GCR_Debug("AddRef pObj2: " & DllStructGetData($pObject, 2))
	; __GCR_Debug("AddRef tData 1: " & DllStructGetData($tData, 1))
	; __GCR_Debug("AddRef tData 2: " & DllStructGetData($tData, 2))
	DllStructSetData($tData, 2, DllStructGetData($tData, 2) + 1)
	; __GCR_Debug("AddRef tData 2 new: " & DllStructGetData($tData, 2))
...
Func __RichCom_Object_Release($pObject)
	;Exit Function
	Local $tData = DllStructCreate("ptr;dword", $pObject)
	; __GCR_Debug("Releasecheck : " & DllStructGetData($tData, 2))
	If DllStructGetData($tData, 2) > 0 Then
		DllStructSetData($tData, 2, DllStructGetData($tData, 2) - 1)
		; __GCR_Debug("Release: " & DllStructGetData($tData, 2))
		Return DllStructGetData($tData, 2)
	EndIf
	;If @pObject[1] > 0 Then
	;Decr @pObject[1]
...
Func __RichCom_Object_GetNewStorage($pObject, $pPstg)
	...
	; __GCR_Debug("GetNewStorage $aSc isobj: " & IsArray($aSc))
	Local $pLockBytes = $aSc[3]
	$aSc = $aSc[0]
	; __GCR_Debug("GetNewStorage $aSc: " & $aSc & " $pLockBytes " & $pLockBytes)
	...
	; __GCR_Debug("GetNewStorage pPstg: " & $pPstg & " $ppstgOpen[4]: " & $aSc[4])
	...
	;, lplpstg _
	;) To sc
	; __GCR_Debug("GetNewStorage 1: " & $aSc)
	; __GCR_Debug("GetNewStorage pLockBytes: " & $pLockBytes)
	If $aSc Then ; Call IUnknown->Release on $pLockBytes
		; __GCR_Debug("GetNewStorage aSc: " & $aSc)
